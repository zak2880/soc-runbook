...
- Staging directory — files being collected and compressed before sending
- `rclone`, `MEGASync`, or similar uploading from unexpected process
...
What ran:        key process chain, e.g. outlook.exe → mshta.exe → powershell.exe -enc
What it did:     network connections, files dropped, persistence created
...
| Scenario | Severity |
|----------|----------|
| Malware detected and quarantined, no execution evidence | Low — review and close |
| Malware executed, no persistence, no spread, no C2 | Medium — full investigation, confirm contained |
...
| Ransomware indicators present | Critical — escalate immediately, consider P1 |
