...
**Platform:** Microsoft Defender XDR Advanced Hunting (native). Time column is `Timestamp`. `EntraIdSignInEvents` is not a Sentinel-native table — the Sentinel equivalent is `SigninLogs` (via the Azure AD / Entra ID connector), which has a different schema. See [kql/sentinel/identity/entra-device-code-auth-flow.md](../../sentinel/identity/entra-device-code-auth-flow.md) for the Sentinel version  
...
| where Timestamp between (ago(baseline_lookback) .. ago(baseline_gap))
| where ErrorCode == 0
...
| project Timestamp, AccountUpn, Country, IPAddress, UserAgent,
