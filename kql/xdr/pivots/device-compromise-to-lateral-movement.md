...
**Platform:** Microsoft Defender XDR Advanced Hunting (native). Time column is `Timestamp`. See [kql/sentinel/pivots/device-compromise-to-lateral-movement.kql](../../sentinel/pivots/device-compromise-to-lateral-movement.kql) for the Sentinel version  
...
| summarize FirstSeen = min(Timestamp) by AccountName;
...
union
    ...
    (InternalIPsAccessed | extend Stage = "2-InternalIPAccessed", TimelineTime = Timestamp),
    (RemoteExecTools | extend Stage = "3-RemoteExecTool", TimelineTime = Timestamp),
    (SpreadToOtherDevices | extend Stage = "4-AccountSpreadToOtherDevice", TimelineTime = Timestamp)
