...
**Platform:** ⚠️ `EmailEvents` and `EmailAttachmentInfo` are Defender XDR Advanced Hunting-only tables — NOT exposed as queryable Sentinel Log Analytics tables, even with the Microsoft Defender XDR connector configured (see [docs/reference/sentinel-vs-advanced-hunting.md](../../../docs/reference/sentinel-vs-advanced-hunting.md)). `DeviceFileEvents` and `DeviceProcessEvents` ARE available in Sentinel via that connector, but the first two stages of this pivot are not. This file is kept for structural parity with [kql/xdr/pivots/phishing-to-device-compromise.kql](../../xdr/pivots/phishing-to-device-compromise.kql) and will error with "table not found" on the `EmailStage` in a real Sentinel workspace today. Time column shown below is `TimeGenerated` to match the rest of this repo's Sentinel tree, but that doesn't change whether the query runs — if you need this pivot in Sentinel, start from an individual MDO alert synced into Sentinel incidents instead of querying `EmailEvents` directly  
...
**When to use:** You've identified a suspicious email (by sender or `NetworkMessageId`) and need to know, in one pass, whether it actually led to code execution on any device — not just delivery. In practice, run [kql/xdr/pivots/phishing-to-device-compromise.kql](../../xdr/pivots/phishing-to-device-compromise.kql) instead — it queries the same logic against tables that actually exist on that platform.
...
// Microsoft Defender XDR connector configured. This query is kept for structural
// parity with kql/xdr/pivots/phishing-to-device-compromise.kql and will error with
// "table not found" on the EmailStage in a real Sentinel workspace today.
